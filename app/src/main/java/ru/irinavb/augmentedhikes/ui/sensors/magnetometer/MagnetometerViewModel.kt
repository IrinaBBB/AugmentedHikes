...
class MagnetometerViewModel @Inject constructor(
    ...
    val magnetometerSensorData: LiveData<List<Float>>
    ...
    fun beginListeningMagnetometer(context: Context) {
    ...
    private fun writeSensorDataToLocalStorage(context: Context, data: List<Float>) {
        val text = "magnetometer ${LocalDateTime.now()}: X -> ${data[0]}, " +
                "Y -> ${data[1]}, Z -> ${data[2]}\n"
    ...
    fun unregisterMagnetometerSensorListener() {
